...
//final float SCALE_F = 3;

class DataHolder {  
  String LOCATION, OPERATION;
  int VALUE, INDEX, WEEKS, YEAR, ID;
  ...
  float parseCoord(String a) {
    ...
    if  (orientation==87 || orientation==83) coord*=-DEG_TO_RAD; 
    else coord*=DEG_TO_RAD;
    return coord;
  }
  ...
  void render(PGraphics canvas, boolean buffered) {
    for (int i=0; i< data.length; i++) {
      ...
      if (data[i].ID == 1) {
        ...
        canvas.line(data[i].START.x, data[i].START.y, data[i].START.z, data[i].END.x, data[i].END.y, data[i].END.z);
        //println (data[i].LOCATION);
      } else {
